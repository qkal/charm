/**
 * db-base — Reusable SQLite infrastructure for charm packages.
 *
...
/**
 * Return the default per-process DB path for charm databases.
 * Uses the OS temp directory and embeds the current PID so multiple
 * server instances never share a file.
 */
export function defaultDBPath(prefix: string = "charm"): string {
 ...
 */
const _kLiveDBs = Symbol.for("__charm_live_dbs__");
