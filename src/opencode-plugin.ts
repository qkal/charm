/**
 * OpenCode TypeScript plugin entry point for charm.
 *
 ...
 *
 * Loaded by OpenCode via: import("charm/plugin").CharmPlugin(ctx)
 *
 * Constraints:
...
*/
export const CharmPlugin = async (ctx: PluginContext) => {
  ...
  return {
    ...
    "tool.execute.before": async (input: BeforeHookInput, output: BeforeHookOutput) => {
      ...
      if (decision.action === "deny" || decision.action === "ask") {
        ...
        throw new Error(decision.reason ?? "Blocked by charm");
      }
