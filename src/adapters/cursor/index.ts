...
export class CursorAdapter implements HookAdapter {
  ...
  formatPreToolUseResponse(response: PreToolUseResponse): unknown {
    if (response.decision === "deny") {
      return {
        permission: "deny",
        user_message: response.reason ?? "Blocked by charm hook",
      };
    }
  ...
  getSessionDir(): string {
    const dir = join(homedir(), ".cursor", "charm", "sessions");
    ...
    if (!loaded) {
      ...
      for (const hookType of REQUIRED_HOOKS) {
        ...
        const hasHook = Array.isArray(entries)
          && entries.some((entry) => isCharmHook(entry, hookType));
  ...
  checkPluginRegistration(): DiagnosticResult {
    ...
    for (const configPath of mcpPaths) {
      try {
        ...
        const hasCharm = Object.entries(servers).some(([name, value]) => {
          if (name.includes("charm")) return true;
          if (!value || typeof value !== "object") return false;
          const server = value as Record<string, unknown>;
          return server.command === "charm";
        });
        ...
        if (hasCharm) {
          return {
            ...
            status: "pass",
            message: `charm found in ${configPath}`,
          };
        }
      } catch {
        continue;
    ...
    return {
      check: "MCP registration",
      status: "warn",
      message: "Could not find charm in .cursor/mcp.json or ~/.cursor/mcp.json",
    };
  }
  ...
  private upsertHookEntry(
    ...
    const idx = existing.findIndex((candidate) => isCharmHook(candidate, hookType));
