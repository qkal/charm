...
/** Canonical Cursor-native matchers for tools charm routes proactively. */
...
/** Check whether a native Cursor hook entry points to charm. */
...
export function buildHookCommand(hookType: HookType): string {
  return `charm hook cursor ${hookType.toLowerCase()}`;
}
