 ...
 *   - Session dir: ~/.openclaw/charm/sessions/
 */
...
export class OpenClawAdapter implements HookAdapter {
  ...
  formatPreToolUseResponse(response: PreToolUseResponse): unknown {
    if (response.decision === "deny") {
      ...
      return {
        block: true,
        blockReason: response.reason ?? "Blocked by charm hook",
      };
    }
  ...
  getSessionDir(): string {
    const dir = join(homedir(), ".openclaw", "charm", "sessions");
  ...
  validateHooks(_pluginRoot: string): DiagnosticResult[] {
    ...
    // Check for charm in plugins.entries
    const plugins = settings.plugins as Record<string, unknown> | undefined;
    const entries = plugins?.entries as Record<string, unknown> | undefined;

    if (entries) {
      const hasPlugin = Object.keys(entries).some((k) => k.includes("charm"));
      results.push({
        check: "Plugin registration",
        status: hasPlugin ? "pass" : "fail",
        message: hasPlugin
          ? "charm found in plugins.entries"
          : "charm not found in plugins.entries",
        fix: hasPlugin
          ? undefined
          : "charm upgrade",
      });
      ...
      // Check if enabled
      if (hasPlugin) {
        const entry = entries["charm"] as Record<string, unknown> | undefined;
        const isEnabled = entry?.enabled !== false;
        results.push({
          check: "Plugin enabled",
          status: isEnabled ? "pass" : "warn",
          message: isEnabled
            ? "charm plugin is enabled"
            : "charm plugin is disabled",
        });
      }
    } else {
      results.push({
    ...
    if (slots?.contextEngine === "charm") {
      results.push({
        check: "Context engine",
        status: "pass",
        message: "charm registered as context engine (owns compaction)",
      });
    } else {
      results.push({
        check: "Context engine",
        status: "warn",
        message:
          "charm not set as context engine — compaction will use default engine",
      });
    }
  ...
  checkPluginRegistration(): DiagnosticResult {
    ...
    if (entries) {
      ...
      if (hasPlugin) {
        return {
          check: "Plugin registration",
          status: "pass",
          message: "charm found in plugins.entries",
        };
      }
    }
    ...
    return {
      check: "Plugin registration",
      status: "fail",
      message: "charm not found in openclaw.json plugins.entries",
      fix: "charm upgrade",
    };
  }

  getInstalledVersion(): string {
    // Check ~/.openclaw/extensions/charm/ for the plugin
    try {
  ...
  configureAllHooks(_pluginRoot: string): string[] {
    ...
    // Add charm to plugins.entries
    if (!entries["charm"]) {
      entries["charm"] = { enabled: true };
      changes.push("Added charm to plugins.entries");
    } else {
      const entry = entries["charm"] as Record<string, unknown>;
      if (entry.enabled === false) {
        entry.enabled = true;
        changes.push("Enabled charm plugin");
      } else {
        changes.push("charm already configured in plugins.entries");
      }
    }
    ...
    if (!slots.contextEngine) {
      slots.contextEngine = "charm";
      changes.push("Set charm as context engine (owns compaction)");
    } else if (slots.contextEngine !== "charm") {
      changes.push(
