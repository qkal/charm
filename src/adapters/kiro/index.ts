 ...
 *   - Session dir: ~/.kiro/charm/sessions/
...
import {
  ...
  isCharmHook as isKiroCharmHook,
} from "./hooks.js";
...
export class KiroAdapter implements HookAdapter {
  ...
  formatPreToolUseResponse(response: PreToolUseResponse): unknown {
    ...
    switch (response.decision) {
      case "deny":
        return { exitCode: 2, stderr: response.reason ?? "Blocked by charm" };
      case "context":
  ...
  getSessionDir(): string {
    const dir = join(homedir(), ".kiro", "charm", "sessions");
    ...
    try {
      ...
      for (const hookType of [KIRO_HOOK_TYPES.PRE_TOOL_USE]) {
        ...
        const found = entries.some((e: { command?: string }) =>
          isKiroCharmHook(e, hookType),
        );
        results.push({
          check: `Hook: ${hookType}`,
          status: found ? "pass" : "fail",
          message: found
            ? `charm ${hookType} hook found`
            : `charm ${hookType} hook not configured`,
          ...(found ? {} : { fix: `Run: charm upgrade` }),
        });
      }
      ...
      for (const hookType of [KIRO_HOOK_TYPES.POST_TOOL_USE]) {
        ...
        results.push({
          ...
          message: found
            ? `charm ${hookType} hook found`
            : `charm ${hookType} hook not configured (optional)`,
        });
      }
    } catch {
      results.push({
  ...
  checkPluginRegistration(): DiagnosticResult {
    try {
      ...
      if ("charm" in mcpServers) {
        return {
          check: "MCP registration",
          status: "pass",
          message: "charm found in mcpServers config",
        };
      }
      ...
      return {
        check: "MCP registration",
        status: "fail",
        message: "charm not found in mcpServers",
        fix: "Add charm to mcpServers in ~/.kiro/settings/mcp.json",
      };
    } catch {
      return {
        check: "MCP registration",
  ...
  configureAllHooks(pluginRoot: string): string[] {
    ...
    try {
      ...
      if (!preToolUseEntries.some(e => isKiroCharmHook(e as { command?: string }, KIRO_HOOK_TYPES.PRE_TOOL_USE))) {
      ...
      if (!postToolUseEntries.some(e => isKiroCharmHook(e as { command?: string }, KIRO_HOOK_TYPES.POST_TOOL_USE))) {
