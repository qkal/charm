 ...
 *   - Session dir: ~/.codex/charm/sessions/
 *
...
export class CodexAdapter implements HookAdapter {
  ...
  getSessionDir(): string {
    const dir = join(homedir(), ".codex", "charm", "sessions");
  ...
  checkPluginRegistration(): DiagnosticResult {
    // Check for charm in [mcp_servers] section of config.toml
    try {
      const raw = readFileSync(this.getSettingsPath(), "utf-8");
      ...
      if (hasCharm && hasMcpSection) {
        return {
          check: "MCP registration",
          status: "pass",
          message: "charm found in [mcp_servers] config",
        };
      }
      ...
      if (hasMcpSection) {
        return {
          ...
          status: "fail",
          message:
            "[mcp_servers] section exists but charm not found",
          fix: 'Add charm to [mcp_servers] in ~/.codex/config.toml',
        };
      }
      ...
      return {
        ...
        fix: 'Add [mcp_servers.charm] to ~/.codex/config.toml',
      };
    } catch {
      return {
        check: "MCP registration",
