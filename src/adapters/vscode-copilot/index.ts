 ...
 *   - Session dir: ~/.vscode/charm/sessions/ (fallback)
...
export class VSCodeCopilotAdapter implements HookAdapter {
  ...
  formatPreToolUseResponse(response: PreToolUseResponse): unknown {
    if (response.decision === "deny") {
      return {
        ...
        reason: response.reason ?? "Blocked by charm hook",
      };
    }
  ...
  getSettingsPath(): string {
    ...
    return resolve(".github", "hooks", "charm.json");
  }

  getSessionDir(): string {
    // Prefer .github/charm/sessions/ if .github exists,
    // otherwise fall back to ~/.vscode/charm/sessions/
    const githubDir = resolve(".github", "charm", "sessions");
  ...
  readSettings(): Record<string, unknown> | null {
    // Try .github/hooks/charm.json first, then .claude/settings.json
    const paths = [
  ...
  validateHooks(pluginRoot: string): DiagnosticResult[] {
    ...
    // Check for charm hook config
    const hookConfigPath = resolve(hooksDir, "charm.json");
    try {
      ...
      if (hooks?.[VSCODE_HOOK_NAMES.PRE_TOOL_USE]) {
        results.push({
          ...
          status: "pass",
          message: "PreToolUse hook configured in charm.json",
        });
      } else {
        results.push({
          check: "PreToolUse hook",
          status: "fail",
          message: "PreToolUse not found in charm.json",
          fix: "charm upgrade",
        });
      }
      ...
      if (hooks?.[VSCODE_HOOK_NAMES.SESSION_START]) {
        results.push({
          ...
          status: "pass",
          message: "SessionStart hook configured in charm.json",
        });
      } else {
        results.push({
          check: "SessionStart hook",
          status: "fail",
          message: "SessionStart not found in charm.json",
          fix: "charm upgrade",
        });
      }
    } catch {
      results.push({
        ...
        status: "fail",
        message: "Could not read .github/hooks/charm.json",
        fix: "charm upgrade",
      });
    }
  ...
  checkPluginRegistration(): DiagnosticResult {
    ...
    try {
      ...
      if (servers) {
        ...
        if (hasPlugin) {
          return {
            ...
            status: "pass",
            message: "charm found in .vscode/mcp.json",
          };
        }
      }
      ...
      return {
        check: "MCP registration",
        status: "fail",
        message: "charm not found in .vscode/mcp.json",
        fix: "Add charm server to .vscode/mcp.json",
      };
    } catch {
      return {
        check: "MCP registration",
  ...
  getInstalledVersion(): string {
    // Check VS Code extensions for charm
    const extensionDirs = [
  ...
  configureAllHooks(pluginRoot: string): string[] {
    ...
    // Write to .github/hooks/charm.json
    const outputDir = resolve(".github", "hooks");
    mkdirSync(outputDir, { recursive: true });
    const outputPath = resolve(outputDir, "charm.json");
    writeFileSync(
      outputPath,
