 ...
 *   - Session dir: ~/.gemini/charm/sessions/
 */
...
export class GeminiCLIAdapter implements HookAdapter {
  ...
  getSessionDir(): string {
    const dir = join(homedir(), ".gemini", "charm", "sessions");
  ...
  validateHooks(pluginRoot: string): DiagnosticResult[] {
    ...
    if (beforeTool && beforeTool.length > 0) {
      const hasHook = beforeTool.some((entry) =>
        entry.hooks?.some((h) => h.command?.includes("charm")),
      );
      results.push({
        check: "BeforeTool hook",
        ...
        message: hasHook
          ...
          : "BeforeTool exists but does not point to charm",
        fix: hasHook ? undefined : "charm upgrade",
      });
    } else {
      results.push({
        check: "BeforeTool hook",
    ...
    if (sessionStart && sessionStart.length > 0) {
      ...
      results.push({
        ...
        message: hasHook
          ...
          : "SessionStart exists but does not point to charm",
        fix: hasHook ? undefined : "charm upgrade",
      });
    } else {
      results.push({
  ...
  checkPluginRegistration(): DiagnosticResult {
    ...
    // Check in extensions or settings for charm
    ...
    if (extensions) {
      const hasPlugin = Array.isArray(extensions)
        ? extensions.some(
            (e) =>
              typeof e === "string" && e.includes("charm"),
          )
        : Object.keys(extensions).some((k) => k.includes("charm"));

      if (hasPlugin) {
        return {
          check: "Plugin registration",
          status: "pass",
          message: "charm found in extensions",
        };
      }
    }
    ...
    return {
      check: "Plugin registration",
      status: "warn",
      message: "charm not found in extensions (might be using standalone MCP mode)",
    };
  }

  getInstalledVersion(): string {
    // Check ~/.gemini/ extension cache for charm
    try {
    ...
    for (const config of hookConfigs) {
      ...
      if (existing && Array.isArray(existing)) {
        const idx = existing.findIndex((e) => {
          ...
          return entryHooks?.some((h) => h.command?.includes("charm"));
        });
        if (idx >= 0) {
          existing[idx] = entry;
