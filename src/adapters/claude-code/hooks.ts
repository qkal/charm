...
/** Tools that charm's PreToolUse hook intercepts. */
...
/**
 * Tools that charm's PostToolUse hook should fire on.
...
/** Required hooks that must be configured for charm to function. */
...
/**
 * Check if a hook entry points to a charm hook script.
 * Matches both legacy format (node .../pretooluse.mjs) and
 * CLI dispatcher format (charm hook claude-code pretooluse).
 */
export function isCharmHook(
...
export function buildHookCommand(hookType: HookType, pluginRoot?: string): string {
  ...
  return `charm hook claude-code ${hookType.toLowerCase()}`;
}
...
/**
 * Check if a hook entry is a charm hook (any hook type).
 * Broader than `isCharmHook` — matches any charm script name
 * without requiring a specific hookType.
 */
export function isAnyCharmHook(
  entry: { hooks?: Array<{ command?: string }> },
): boolean {
