 ...
 *   - Session dir: ~/.claude/charm/sessions/
 */
...
export class ClaudeCodeAdapter implements HookAdapter {
  ...
  validateHooks(pluginRoot: string): DiagnosticResult[] {
    ...
    results.push({
      ...
      fix: hasPreToolUse ? undefined : "charm upgrade",
    });
    ...
    results.push({
      ...
      fix: hasSessionStart ? undefined : "charm upgrade",
    });
  ...
  private checkHookType(
    ...
    if (fromSettings && fromSettings.length > 0) {
      if (fromSettings.some((entry) => isCharmHook(entry, hookType))) {
        return true;
      }
    }
    ...
    if (fromPlugin && fromPlugin.length > 0) {
      if (fromPlugin.some((entry) => isCharmHook(entry, hookType))) {
        return true;
      }
    }
  ...
  checkPluginRegistration(): DiagnosticResult {
    ...
    return {
      check: "Plugin registration",
      status: "warn",
      message: "charm not in enabledPlugins (might be using standalone MCP mode)",
    };
  }
  ...
  getInstalledVersion(): string {
    ...
    try {
      ...
      for (const [key, entries] of Object.entries(plugins)) {
        if (!key.toLowerCase().includes("charm")) continue;
  ...
  configureAllHooks(pluginRoot: string): string[] {
    ...
    // Remove stale charm hook entries across ALL hook types (fixes #187).
    ...
    for (const hookType of Object.keys(hooks)) {
      ...
      const filtered = entries.filter((entry: Record<string, unknown>) => {
        ...
        if (!isAnyCharmHook(typedEntry)) return true; // preserve non-charm hooks
    ...
    if (pluginHooks) {
      ...
      if (allCovered) {
        // Remove ALL existing charm hooks from settings.json — hooks.json
        ...
        for (const hookType of Object.keys(hooks)) {
          ...
          const filtered = (entries as Array<Record<string, unknown>>).filter((entry) =>
            !isAnyCharmHook(entry as { hooks?: Array<{ command?: string }> }),
          );
          const removed = entries.length - filtered.length;
          if (removed > 0) {
            hooks[hookType] = filtered;
      ...
      if (hookType === HOOK_TYPES.PRE_TOOL_USE) {
        ...
        if (existing && Array.isArray(existing)) {
          const idx = existing.findIndex((e) =>
            isCharmHook(e as { hooks?: Array<{ command?: string }> }, hookType),
          );
          if (idx >= 0) {
            existing[idx] = entry;
