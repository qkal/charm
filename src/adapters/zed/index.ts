 ...
 *   - Session dir: ~/.config/zed/charm/sessions/
 */
...
export class ZedAdapter implements HookAdapter {
  ...
  getSessionDir(): string {
    const dir = join(homedir(), ".config", "zed", "charm", "sessions");
  ...
  checkPluginRegistration(): DiagnosticResult {
    // Check for charm in context_servers section of settings.json
    try {
      const raw = readFileSync(this.getSettingsPath(), "utf-8");
      const settings = JSON.parse(raw);
      const hasContextServers = settings.context_servers !== undefined;
      const hasCharm = raw.includes("charm");

      if (hasContextServers && hasCharm) {
        return {
          check: "MCP registration",
          status: "pass",
          message: "charm found in context_servers config",
        };
      }
      ...
      if (hasContextServers) {
        return {
          ...
          status: "fail",
          message:
            "context_servers section exists but charm not found",
          fix: 'Add charm to context_servers in ~/.config/zed/settings.json',
        };
      }
      ...
      return {
        ...
        fix: 'Add context_servers.charm to ~/.config/zed/settings.json',
      };
    } catch {
      return {
        check: "MCP registration",
  ...
  getInstalledVersion(): string {
    // Zed has no marketplace or plugin system for charm
  ...
  getRoutingInstructions(): string {
    ...
    const instructions = readRoutingPrompt("zed", pluginRoot);
