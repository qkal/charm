...
/**
 * Required hooks that must be active for charm to function.
...
/**
 * Check if an OpenCode plugin entry is the charm plugin.
 * OpenCode plugins are registered as strings in the plugin array.
 */
export function isCharmPlugin(pluginEntry: string): boolean {
  return pluginEntry.includes("charm");
}
