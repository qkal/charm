 ...
 *   - Session dir: ~/.config/opencode/charm/sessions/
 */
...
export class OpenCodeAdapter implements HookAdapter {
  ...
  formatPreToolUseResponse(response: PreToolUseResponse): unknown {
    if (response.decision === "deny") {
      ...
      throw new Error(
        response.reason ?? "Blocked by charm hook",
      );
    }
  ...
  getSessionDir(): string {
    ...
    const dir = join(configDir, this.platform, "charm", "sessions");
  ...
  readSettings(): Record<string, unknown> | null {
    ...
    for (const configPath of configPaths) {
      try {
        ...
        if (this.hasCharmPlugin(settings) || isGlobalConfig) {
  ...
  validateHooks(_pluginRoot: string): DiagnosticResult[] {
    ...
    // Check for "charm" in plugin array
    const hasPlugin = this.hasCharmPlugin(settings);
    if (Array.isArray(settings.plugin)) {
      results.push({
        check: "Plugin registration",
        status: hasPlugin ? "pass" : "fail",
        message: hasPlugin
          ? "charm found in plugin array"
          : "charm not found in plugin array",
        fix: hasPlugin
          ? undefined
          : "charm upgrade",
      });
  ...
  checkPluginRegistration(): DiagnosticResult {
    ...
    if (this.hasCharmPlugin(settings)) {
      return {
        check: "Plugin registration",
        status: "pass",
        message: "charm found in plugin array",
      };
    }
    ...
    return {
      check: "Plugin registration",
      status: "fail",
      message: `charm not found in ${this.platform}.json plugin array`,
      fix: "charm upgrade",
    };
  }

  getInstalledVersion(): string {
    // Check ~/.cache/opencode/node_modules/ for charm
    try {
  ...
  configureAllHooks(_pluginRoot: string): string[] {
    ...
    // Add "charm" to the plugin array
    const plugins = (settings.plugin ?? []) as string[];
    if (!plugins.some((p) => p.includes("charm"))) {
      plugins.push("charm");
      changes.push("Added charm to plugin array");
    } else {
      changes.push("charm already in plugin array");
    }
  ...
  /**
   * Check whether a settings object has the charm plugin registered.
   */
  private hasCharmPlugin(settings: Record<string, unknown>): boolean {
    const plugins = settings.plugin;
    return Array.isArray(plugins) && plugins.some((p: unknown) => typeof p === "string" && p.includes("charm"));
  }
