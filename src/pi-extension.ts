/**
 * Pi coding agent extension for charm.
 *
...
function getSessionDir(): string {
  const dir = join(homedir(), ".pi", "charm", "sessions");
  mkdirSync(dir, { recursive: true });
  return dir;
}

function getDBPath(): string {
  return join(getSessionDir(), "charm.db");
}
    ...
    try {
      ...
      if (isBlocked) {
        return {
          block: true,
          reason:
            "Use charm MCP tools (execute, fetch_and_index) instead of inline HTTP clients. " +
