...
export function formatReport(report: FullReport, version?: string, latestVersion?: string | null): string {
  ...
  if (totalKeptOut === 0) {
    lines.push(`charm -- session (${duration})`);
    lines.push("");
    ...
    lines.push("");
    lines.push(version ? `v${version}` : "charm");
  ...
  lines.push(`Without charm:  |${withoutBar}| ${kb(grandTotal)} in your conversation`);
  lines.push(`With charm:     |${withBar}| ${kb(totalReturned)} in your conversation`);
  lines.push("");
  ...
  const versionStr = version ? `v${version}` : "charm";
