...
process.on("unhandledRejection", (err) => {
  process.stderr.write(`[charm] unhandledRejection: ${err}\n`);
});
process.on("uncaughtException", (err) => {
  process.stderr.write(`[charm] uncaughtException: ${err?.message ?? err}\n`);
});
 ...
 * Scans ~/.claude/charm/sessions/ for *-events.md files.
 ...
 * Falls back to ~/.claude/charm/sessions/ before adapter detection.
 */
function getSessionDir(): string {
  if (_detectedAdapter) return _detectedAdapter.getSessionDir();
  const dir = join(homedir(), ".claude", "charm", "sessions");
 ...
 *   2. CHARM_PROJECT_DIR (set by start.mjs for ALL platforms — universal)
 *   3. process.cwd() (last resort)
 *
 * CHARM_PROJECT_DIR guarantees correct projectDir even for platforms
 ...
 *
 * Layout: ~/<configDir>/charm/content/<hash>.db
 *   e.g.  ~/.claude/charm/content/87c28c41ddb64d38.db
 *         ~/.cursor/charm/content/87c28c41ddb64d38.db
 */
...
function getStore(): ContentStore {
  if (!_store) {
    ...
    try {
      ...
      const legacyDir = join(homedir(), ".charm", "content");
...
async function fetchLatestVersion(): Promise<string> {
  return new Promise((res) => {
    const req = httpsRequest(
      "https://registry.npmjs.org/charm/latest",
...
function trackResponse(toolName: string, response: ToolResult): ToolResult {
  ...
  if (shouldShowVersionWarning() && response.content.length > 0) {
    ...
    response.content[0].text =
      `⚠️ charm v${VERSION} outdated → v${_latestVersion} available. Upgrade: ${hint}\n\n` +
      response.content[0].text;
  }
...
async function runExecuteWithInstrumentation(args: {
  language: Language;
  code: string;
  timeout: number;
  background: boolean;
}): Promise<ExecResult> {
  const { language, code, timeout, background } = args;
...
__cm_main().catch(e=>{console.error(e);process.exitCode=1});${background ? '\nsetInterval(()=>{},2147483647);' : ''}
  ...
  parseSandboxMetrics(result);
  return result;
}

async function runExecuteFileRaw(args: {
  path: string;
  language: Language;
  code: string;
  timeout: number;
  ...
  return executor.executeFile({ path, language, code, timeout });
}

function parseSandboxMetrics(result: ExecResult): void {
...
const executionEffects = new ExecutionEffects({
  classifyNonZeroExit,
  trackResponse,
  trackIndexed,
  intentSearch,
...
const executionService = new ExecutionService({
  runExecute: runExecuteWithInstrumentation,
  runExecuteFile: runExecuteFileRaw,
  checkDenyPolicy,
...
server.registerTool(
  ...
  async ({ path, language, code, timeout, intent }) => {
    return executionService.executeFile({
      path,
      language,
      code,
      timeout,
...
server.registerTool(
  "ctx_doctor",
  {
    ...
    description:
      "Diagnose charm installation. Runs all checks server-side and " +
...
server.registerTool(
  ...
  async () => {
    const text = await buildUpgradeMessage({ pkgDir: __pkg_dir });
...
server.registerTool(
  ...
  async ({ confirm }) => {
    ...
    // 2. Wipe legacy shared content DB (~/.charm/content/<hash>.db)
    try {
      const legacyPath = join(homedir(), ".charm", "content", `${hashProjectDir()}.db`);
      for (const suffix of ["", "-wal", "-shm"]) {
...
server.registerTool(
  "ctx_insight",
  {
    ...
    description:
      "Opens the charm Insight dashboard in the browser. " +
  ...
  async ({ port: userPort }) => {
    ...
    if (!existsSync(join(insightSource, "server.mjs"))) {
      return trackResponse("ctx_insight", {
        content: [{ type: "text" as const, text: "Error: Insight source not found in plugin. Try upgrading charm." }],
      });
    }
...
async function main() {
  ...
  const mcpSentinel = join(tmpdir(), `charm-mcp-ready-${process.ppid}`);
  ...
  console.error(`Charm MCP server v${VERSION} running on stdio`);
