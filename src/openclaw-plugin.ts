/**
 * OpenClaw TypeScript plugin entry point for charm.
 *
 ...
 *   - charm engine      — Context engine with compaction management
...
const configSchema = {
  ...
  properties: {
    enabled: {
      ...
      default: true,
      description: "Enable or disable the charm plugin.",
    },
  },
...
export default {
  ...
  register(api: OpenClawPluginApi): void {
    ...
    const log = {
      info: (...args: unknown[]) => api.logger?.info("[charm]", ...args),
      error: (...args: unknown[]) => api.logger?.error("[charm]", ...args),
      debug: (...args: unknown[]) => api.logger?.debug?.("[charm]", ...args),
      warn: (...args: unknown[]) => api.logger?.warn?.("[charm]", ...args),
    };
    ...
    api.on(
      ...
      async (event: unknown) => {
        ...
        if (decision.action === "deny" || decision.action === "ask") {
          return {
            block: true,
            blockReason: decision.reason ?? "Blocked by charm",
          };
        }
    ...
    api.registerContextEngine("charm", () => ({
      info: {
        id: "charm",
        name: "Charm",
      ...
      async compact() {
        ...
        // See: https://github.com/lovablia/charm/issues/191
    ...
    if (api.registerCommand) {
      api.registerCommand({
        name: "ctx-stats",
        description: "Show charm session statistics",
        handler: () => {
      ...
      api.registerCommand({
        name: "ctx-doctor",
        description: "Run charm diagnostics",
        handler: () => {
          ...
          return {
            text: [
              "## ctx-doctor",
              "",
              "Run this command to diagnose charm:",
              "",
              "```",
              cmd,
              "```",
...
function buildStatsText(db: SessionDB, sessionId: string): string {
  try {
    ...
    return "charm stats unavailable (session DB error)";
  }
}
