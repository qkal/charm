...
/* -------------------------------------------------------
 * Hook dispatcher — `charm hook <platform> <event>`
 * ------------------------------------------------------- */
...
function cachePluginRoot(platform: string): string {
  if (process.platform === "win32") {
    ...
    if (localApp) return resolve(localApp, platform, "node_modules", "charm");
    return resolve(homedir(), "AppData", "Local", platform, "node_modules", "charm");
  }
  return resolve(homedir(), ".cache", platform, "node_modules", "charm");
}
...
async function doctor(): Promise<number> {
  ...
  p.intro(color.bgMagenta(color.white(" charm doctor ")));
  p.log.info(
...
async function insight(port: number) {
  ...
  if (!existsSync(join(insightSource, "server.mjs"))) {
    console.error("Error: Insight source not found. Try upgrading charm.");
    process.exit(1);
  }
  ...
  try {
    ...
    console.error(`  Or use a different port:   charm insight ${port + 1}`);
    child.kill();
    process.exit(1);
  }
...
async function upgrade() {
  ...
  p.intro(color.bgCyan(color.black(" charm upgrade ")));
  p.log.info(
    `Platform: ${color.cyan(adapter.name)}` +
  ...
  const localVersion = getLocalVersion();
  const tmpDir = join(tmpdir(), `charm-upgrade-${Date.now()}`);

  s.start("Cloning lovablia/charm");
  try {
    execFileSync(
      "git", ["clone", "--depth", "1", "https://github.com/lovablia/charm.git", tmpDir],
    ...
    try {
      ...
      if (existsSync(registryPath)) {
        ...
        const entries = registry?.plugins?.["charm@charm"];
