 ...
 *
 * Benchmarks charm against REAL outputs from popular Claude Code
...
async function main() {
  ...
  console.log(
    "\u2551" +
      "   Charm — Ecosystem Benchmark (Real MCP & Skill Outputs)        " +
      "\u2551",
  );
  console.log(
    "\u255A" + "\u2550".repeat(74) + "\u255D",
  ...
  console.log(
    `  WITHOUT charm: ${(workflowRaw / 1024).toFixed(1)}KB → ${wfRawTokens.toLocaleString()} tokens (${((wfRawTokens / 200_000) * 100).toFixed(1)}% of context GONE)`,
  );
  console.log(
    `  WITH charm:    ${(workflowCtx / 1024).toFixed(1)}KB → ${wfCtxTokens.toLocaleString()} tokens (${((wfCtxTokens / 200_000) * 100).toFixed(2)}% of context)`,
  );
  console.log(
