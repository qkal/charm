...
describe("getWorktreeSuffix", () => {
  ...
  it("returns empty string when CHARM_SESSION_SUFFIX is empty", () => {
    vi.stubEnv("CHARM_SESSION_SUFFIX", "");
    expect(getWorktreeSuffix()).toBe("");
  });

  it("returns __<value> when CHARM_SESSION_SUFFIX is set", () => {
    vi.stubEnv("CHARM_SESSION_SUFFIX", "my-worktree");
