/**
 * Charm v0.4.0 — Comprehensive Live Benchmark
...
async function main() {
  ...
  const rawNpmLs = await executor.execute({
    language: "shell",
    code: "cd /Users/lovablia/Server/charm-claude-code-plugin/charm && npm ls 2>/dev/null || true",
  });
  ...
  await benchmark("execute", "npm ls → dependency summary", rawNpmLsBytes, async () => {
    const result = await executor.execute({
      language: "shell",
      code: 'cd /Users/lovablia/Server/charm-claude-code-plugin/charm && echo "Dependencies: $(npm ls --depth=0 2>/dev/null | grep -c "├\\|└") direct, $(npm ls --all 2>/dev/null | wc -l | tr -d " ") total"',
    });
