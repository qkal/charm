...
function createStore(): ContentStore {
  const path = join(
    tmpdir(),
    `charm-test-${Date.now()}-${Math.random().toString(36).slice(2)}.db`,
  );
...
describe("DB Cleanup", () => {
  test("cleanupStaleDBs removes files for dead PIDs", () => {
    const fakePid = 99999;
    const fakePath = join(tmpdir(), `charm-${fakePid}.db`);
  ...
  test("cleanupStaleDBs does not remove current process DB", () => {
    const myPath = join(tmpdir(), `charm-${process.pid}.db`);
  ...
  test("store.cleanup() removes own DB and WAL/SHM files", () => {
    ...
    const knownPath = join(tmpdir(), `charm-cleanup-test-${Date.now()}.db`);
  ...
  test("store.cleanup() is safe to call multiple times", () => {
    const path = join(tmpdir(), `charm-cleanup-idempotent-${Date.now()}.db`);
