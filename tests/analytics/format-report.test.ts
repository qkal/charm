 ...
 * 6. Name is "charm", not "Think in Code"
...
describe("formatReport", () => {
  describe("fresh session (no savings)", () => {
    it("shows 'no savings yet' when totalKeptOut is 0 and no calls", () => {
      ...
      expect(output).toContain("charm -- session");
    ...
    it("shows call count and bytes when calls exist but no savings", () => {
      ...
      expect(output).not.toContain("Without charm");
    });
  ...
  describe("active session (before/after comparison)", () => {
    it("shows before/after comparison bars", () => {
      ...
      expect(output).toContain("Without charm:");
      expect(output).toContain("With charm:");
    ...
    it("before bar is always full, after bar is proportional", () => {
      ...
      const output = formatReport(report);
      const withoutLine = output.split("\n").find((l) => l.includes("Without charm"));
      const withLine = output.split("\n").find((l) => l.includes("With charm"));
  ...
  describe("output constraints", () => {
    it("uses 'charm' as name, not 'Think in Code'", () => {
      const report = makeReport({
        savings: {
          ...makeReport().savings,
          total_calls: 10,
  ...
  describe("version handling", () => {
    ...
    it("falls back to 'charm' when version not provided", () => {
      const report = makeReport();
      const output = formatReport(report);
      expect(output).toContain("charm");
    });
