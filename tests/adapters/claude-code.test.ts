...
describe("ClaudeCodeAdapter", () => {
  ...
  describe("config paths", () => {
    ...
    it("session dir is under ~/.claude/charm/sessions/", () => {
      const sessionDir = adapter.getSessionDir();
      expect(sessionDir).toBe(
        join(homedir(), ".claude", "charm", "sessions"),
      );
    });
    ...
    it("creates session dirs under fake HOME instead of the contributor real HOME", () => {
      ...
      expect(sessionDir.startsWith(join(realHome, ".claude", "charm"))).toBe(false);
    });
    ...
    it("DB path uses sha256 hash of projectDir", () => {
      ...
      expect(dbPath).toBe(
        join(homedir(), ".claude", "charm", "sessions", `${hash}.db`),
      );
    });
  });
      ...
      writeFileSync(
        ...
        JSON.stringify({
          hooks: {
            PreToolUse: [{
              matcher: "Bash",
              hooks: [{ type: "command", command: "charm hook claude-code pretooluse" }],
            }],
            SessionStart: [{
              matcher: "",
              hooks: [{ type: "command", command: "charm hook claude-code sessionstart" }],
            }],
          },
        }),
      );
  ...
  describe("configureAllHooks", () => {
    ...
    it("preserves non-charm hooks from other plugins", () => {
      const staleRoot = "/tmp/non-existent-old-version-dir";
      ...
      // Should have the other plugin's hook + the fresh charm hook
    ...
    it("removes existing valid charm hooks from settings.json when plugin hooks.json covers all required hooks", () => {
      ...
      // settings.json has VALID (non-stale) charm hooks — paths exist, so they won't be
