  ...
  describe("config paths", () => {
    ...
    it("uses a dedicated Cursor session dir", () => {
      expect(adapter.getSessionDir()).toBe(
        join(homedir(), ".cursor", "charm", "sessions"),
      );
    });
  });
    ...
    it("validates native project hooks before compatibility fallbacks", () => {
      ...
      writeFileSync(
        ...
        JSON.stringify({
          version: 1,
          hooks: {
            preToolUse: [{ type: "command", command: "charm hook cursor pretooluse" }],
            sessionStart: [{ type: "command", command: "charm hook cursor sessionstart" }],
          },
        }, null, 2),
      );
