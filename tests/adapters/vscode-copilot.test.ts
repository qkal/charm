...
describe("VSCodeCopilotAdapter", () => {
  ...
  describe("config paths", () => {
    it("settings path is .github/hooks/charm.json", () => {
      expect(adapter.getSettingsPath()).toBe(
        resolve(".github", "hooks", "charm.json"),
      );
    });

    it("session dir is under ~/.vscode/charm/sessions/ or .github/", () => {
      // The adapter uses .github/ if it exists, otherwise ~/.vscode/
      // We just verify it returns a valid path containing charm/sessions
      const sessionDir = adapter.getSessionDir();
      expect(sessionDir).toContain("charm");
