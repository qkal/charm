...
describe("detectPlatform", () => {
  ...
  beforeEach(() => {
    ...
    delete process.env.CHARM_PLATFORM;
    vi.restoreAllMocks();
  });
  ...
  // ── CHARM_PLATFORM override ──────────────────────

  it("returns antigravity when CHARM_PLATFORM=antigravity", () => {
    process.env.CHARM_PLATFORM = "antigravity";
    const signal = detectPlatform();
    expect(signal.platform).toBe("antigravity");
    expect(signal.confidence).toBe("high");
    expect(signal.reason).toContain("CHARM_PLATFORM");
  });

  it("returns kiro when CHARM_PLATFORM=kiro", () => {
    process.env.CHARM_PLATFORM = "kiro";
    const signal = detectPlatform();
    expect(signal.platform).toBe("kiro");
    expect(signal.confidence).toBe("high");
    expect(signal.reason).toContain("CHARM_PLATFORM");
  ...
  it("CHARM_PLATFORM takes priority over env vars", () => {
    process.env.CHARM_PLATFORM = "antigravity";
    process.env.CLAUDE_PROJECT_DIR = "/some/project";
    const signal = detectPlatform();
    expect(signal.platform).toBe("antigravity");
  ...
  it("clientInfo takes priority over CHARM_PLATFORM", () => {
    process.env.CHARM_PLATFORM = "codex";
    const signal = detectPlatform({ name: "antigravity-client", version: "1.0" });
    expect(signal.platform).toBe("antigravity");
  });

  it("invalid CHARM_PLATFORM is ignored", () => {
    process.env.CHARM_PLATFORM = "not-a-platform";
    process.env.CLAUDE_PROJECT_DIR = "/some/project";
    const signal = detectPlatform();
    expect(signal.platform).toBe("claude-code");
  });
