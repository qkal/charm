  ...
  describe("config paths", () => {
    ...
    it("session dir is under platform-specific config directory", () => {
      ...
      if (process.platform === "win32") {
        ...
        expectedDir = join(configDir, "opencode", "charm", "sessions");
      } else {
    ...
    it("configureAllHooks writes back to the global config it read", () => {
      ...
      expect(JSON.parse(run.stdout)).toEqual({
        backup: file + ".bak",
        changes: ["Added charm to plugin array"],
      });
      expect(() => readFileSync(resolve(dir, "opencode.json"), "utf-8")).toThrow();
      expect(JSON.parse(readFileSync(file, "utf-8"))).toEqual({ plugin: ["charm"] });

      rmSync(root, { recursive: true, force: true });
    });

    it("readSettings prioritizes config with charm plugin", () => {
      const root = mkdtempSync(join(tmpdir(), "opencode-adapter-"));
      const dir = join(root, "project");
      const home = join(root, "home");
      const conf = join(home, ".config", "opencode");
      ...
      writeFileSync(resolve(dir, "opencode.json"), JSON.stringify({ plugin: ["charm"] }, null, 2) + "\n");
      const run = spawnSync(
        process.execPath,
        [
          tsx,
...
`,
      ...
      expect(run.status).toBe(0);
      expect(JSON.parse(run.stdout)).toEqual({ plugin: ["charm"], version: "1.0" });
      rmSync(root, { recursive: true, force: true });
    });
...
`,
      ...
      expect(run.status).toBe(0);
      expect(JSON.parse(run.stdout)).toEqual(["Added charm to plugin array"]);
...
describe("OpenCodeAdapter for KiloCode", () => {
  ...
  describe("config paths", () => {
    ...
    it("session dir is under platform-specific config directory", () => {
      ...
      if (process.platform === "win32") {
        const configDir = process.env.APPDATA || join(homedir(), "AppData", "Roaming");
        expectedDir = join(configDir, "kilo", "charm", "sessions");
      } else {
        const configDir = process.env.XDG_CONFIG_HOME || join(homedir(), ".config");
        expectedDir = join(configDir, "kilo", "charm", "sessions");
      }
