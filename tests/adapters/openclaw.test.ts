...
describe("OpenClawAdapter", () => {
  ...
  describe("formatPreToolUseResponse", () => {
    ...
    it("returns block object with default message when no reason for deny", () => {
      ...
      expect(result).toEqual({
        block: true,
        blockReason: "Blocked by charm hook",
      });
    });
  ...
  describe("config paths", () => {
    ...
    it("session dir is under ~/.openclaw/charm/sessions/", () => {
      const sessionDir = adapter.getSessionDir();
      expect(sessionDir).toBe(
        join(homedir(), ".openclaw", "charm", "sessions"),
      );
    });
