 ...
 *   WITHOUT charm: raw file content (every byte enters context)
 *   WITH charm: only the printed summary enters context
 *
...
async function main() {
  ...
  console.log(
    "\u2551" +
      "     Charm — Before vs After Comparison (Fixtures)      " +
      "\u2551",
  );
  console.log(
    "\u255A" +
  ...
  console.log(
    `  WITHOUT charm: ${(debugRaw / 1024).toFixed(1)}KB → ${Math.ceil(debugRaw / 4).toLocaleString()} tokens (${((debugRaw / 4 / 200_000) * 100).toFixed(1)}% of context GONE)`,
  );
  console.log(
    `  WITH charm:    ${(debugCtx / 1024).toFixed(1)}KB → ${Math.ceil(debugCtx / 4).toLocaleString()} tokens (${((debugCtx / 4 / 200_000) * 100).toFixed(2)}% of context)`,
  );
  console.log(
