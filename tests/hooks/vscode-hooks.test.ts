...
describe("VS Code Copilot hooks", () => {
  ...
  beforeAll(() => {
    ...
    const sessionsDir = join(homedir(), ".vscode", "charm", "sessions");
  ...
  beforeEach(() => {
    ...
    const guidanceDir = resolve(tmpdir(), `charm-guidance-${suffix}`);
  ...
  describe("pretooluse.mjs", () => {
    ...
    test("run_in_terminal: curl is redirected to echo", () => {
      ...
      expect(result.exitCode).toBe(0);
      const out = JSON.parse(result.stdout);
      expect(out.hookSpecificOutput.updatedInput.command).toContain("charm");
  ...
  describe("sessionstart.mjs", () => {
    test("startup: outputs routing block", () => {
      ...
      expect(result.stdout).toContain("charm");
    });
