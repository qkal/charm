...
describe("Cursor hooks", () => {
  ...
  beforeAll(() => {
    ...
    const sessionsDir = join(homedir(), ".cursor", "charm", "sessions");
    dbPath = join(sessionsDir, `${hash}.db`);
    eventsPath = join(sessionsDir, `${hash}-events.md`);
    realDbPath = join(realHome, ".cursor", "charm", "sessions", `${hash}.db`);
  });
  ...
  describe("sessionstart.mjs", () => {
    test("returns native additional_context JSON", () => {
      ...
      expect(result.exitCode).toBe(0);
      const payload = JSON.parse(result.stdout) as Record<string, unknown>;
      expect(String(payload.additional_context)).toContain("charm");
    });
