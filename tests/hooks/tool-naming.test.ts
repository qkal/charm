...
const mcpSentinel = resolve(tmpdir(), `charm-mcp-ready-${process.ppid}`);
...
describe("getToolName", () => {
  ...
  it("falls back to claude-code for unknown platforms", () => {
    expect(getToolName("unknown-platform", "ctx_search")).toBe(
      "mcp__plugin_charm_charm__ctx_search",
    );
  });
});
...
describe("createToolNamer", () => {
  it("returns a function that produces correct names", () => {
    ...
    expect(t("ctx_execute")).toBe("mcp__charm__ctx_execute");
    expect(t("ctx_search")).toBe("mcp__charm__ctx_search");
  });
});
...
describe("createRoutingBlock", () => {
  it("produces block with platform-specific tool names for gemini-cli", () => {
    ...
    expect(block).toContain("mcp__charm__ctx_batch_execute");
    expect(block).toContain("mcp__charm__ctx_search");
    expect(block).toContain("mcp__charm__ctx_execute");
    expect(block).toContain("mcp__charm__ctx_fetch_and_index");
    // Must NOT contain claude-code prefix
    expect(block).not.toContain("mcp__plugin_charm_charm__");
  });
...
describe("createReadGuidance", () => {
  it("uses kiro-style tool names for kiro platform", () => {
    ...
    expect(guidance).toContain("@charm/ctx_execute_file");
  });
});
...
describe("createGrepGuidance", () => {
  it("uses opencode-style tool names for opencode platform", () => {
    ...
    expect(guidance).toContain("charm_ctx_execute");
  });
});
...
describe("createBashGuidance", () => {
  it("uses zed-style tool names for zed platform", () => {
    ...
    expect(guidance).toContain("mcp:charm:ctx_batch_execute");
    expect(guidance).toContain("mcp:charm:ctx_execute");
  });
});
...
describe("backward compat static exports", () => {
  it("ROUTING_BLOCK uses claude-code naming", () => {
    expect(ROUTING_BLOCK).toContain(
      "mcp__plugin_charm_charm__ctx_batch_execute",
    );
    expect(ROUTING_BLOCK).toContain(
      "mcp__plugin_charm_charm__ctx_search",
    );
  ...
  it("READ_GUIDANCE uses claude-code naming", () => {
    expect(READ_GUIDANCE).toContain(
      "mcp__plugin_charm_charm__ctx_execute_file",
    );
  });

  it("GREP_GUIDANCE uses claude-code naming", () => {
    expect(GREP_GUIDANCE).toContain(
      "mcp__plugin_charm_charm__ctx_execute",
    );
  });
...
describe("routePreToolUse with platform parameter", () => {
  it("curl block message uses gemini-cli tool names when platform=gemini-cli", () => {
    ...
    expect(cmd).toContain("mcp__charm__ctx_fetch_and_index");
    expect(cmd).toContain("mcp__charm__ctx_execute");
    expect(cmd).not.toContain("mcp__plugin_charm_charm__");
  });
  ...
  it("curl block message uses claude-code tool names when platform is omitted", () => {
    ...
    expect(result).not.toBeNull();
    const cmd = (result!.updatedInput as Record<string, string>).command;
    expect(cmd).toContain("mcp__plugin_charm_charm__ctx_fetch_and_index");
  });
  ...
  it("WebFetch deny uses kiro tool names when platform=kiro", () => {
    ...
    expect(result!.reason).toContain("@charm/ctx_fetch_and_index");
    expect(result!.reason).toContain("@charm/ctx_search");
  });
  ...
  it("Read guidance uses vscode-copilot tool names when platform=vscode-copilot", () => {
    ...
    expect(result!.additionalContext).toContain("charm_ctx_execute_file");
    expect(result!.additionalContext).not.toContain("mcp__plugin_charm_charm__");
  });
  ...
  it("Grep guidance uses zed tool names when platform=zed", () => {
    ...
    expect(result).not.toBeNull();
    expect(result!.action).toBe("context");
    expect(result!.additionalContext).toContain("mcp:charm:ctx_execute");
  });
