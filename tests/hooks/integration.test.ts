...
const _guidanceDir = resolve(tmpdir(), `charm-guidance-${_guidanceSuffix}`);

// MCP readiness sentinel — subprocess hooks check process.ppid (= this test's pid)
const mcpSentinel = resolve(tmpdir(), `charm-mcp-ready-${process.pid}`);
...
describe("Read", () => {
  test("Read + file_path: hookSpecificOutput with additionalContext nudge", () => {
    ...
    const parsed = JSON.parse(result.stdout);
    assert.ok(
      parsed.hookSpecificOutput.additionalContext.includes("charm"),
      "Expected nudge to mention charm",
    );
    assert.ok(
      parsed.hookSpecificOutput.additionalContext.includes("<context_guidance>"),
...
describe("Security Policy Enforcement", () => {
  ...
  test("Security: MCP execute + shell + sudo denied", () => {
    const result = runHook(
      {
        tool_name: "mcp__plugin_charm_charm__ctx_execute",
  ...
  test("Security: MCP execute_file + .env path denied", () => {
    const result = runHook(
      {
        tool_name: "mcp__plugin_charm_charm__ctx_execute_file",
  ...
  test("Security: MCP batch_execute with sudo in one command denied", () => {
    const result = runHook(
      {
        tool_name: "mcp__plugin_charm_charm__ctx_batch_execute",
        tool_input: {
          commands: [
...
describe("Plugin Tool Name Format in ROUTING_BLOCK", () => {
  ...
  // For charm: mcp__plugin_charm_charm__<tool-name>
  // The short form mcp__charm__* only works for direct MCP registration.

  const PLUGIN_PREFIX = "mcp__plugin_charm_charm__";
  const SHORT_PREFIX = "mcp__charm__";
...
describe("Skill Commands", () => {
  ...
  test("ctx-doctor skill directory exists with valid SKILL.md", () => {
    ...
    assert.ok(content.includes("/charm:ctx-doctor"), "Trigger must reference ctx-doctor");
  });
  ...
  test("ctx-upgrade skill directory exists with valid SKILL.md", () => {
    ...
    assert.ok(content.includes("/charm:ctx-upgrade"), "Trigger must reference ctx-upgrade");
  });
  ...
  test("ctx-stats skill directory exists with valid SKILL.md", () => {
    ...
    assert.ok(content.includes("/charm:ctx-stats"), "Trigger must reference ctx-stats");
  });
