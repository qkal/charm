 ...
 *
 * Tests the CharmPlugin factory and its three hooks:
...
async function createTestPlugin(tempDir: string) {
  ...
  const { CharmPlugin } = await import("../src/opencode-plugin.js");
  ...
  return CharmPlugin({ directory: tempDir });
}
  ...
  describe("tool.execute.before", () => {
    it("modifies curl commands to block them", async () => {
      ...
      try {
        ...
        expect(output.args.command).toContain("charm");
      } catch (e: any) {
        // deny/ask action throws — still correct behavior
        expect(e.message).toContain("charm");
      }
    });
  ...
  describe("end-to-end flow", () => {
    ...
    it("blocked tool command is replaced before execution", async () => {
      ...
      try {
        ...
        expect(beforeOutput.args.command).toContain("charm");
      } catch (e: any) {
        // deny action throws
        blocked = true;
        expect(e.message).toContain("charm");
