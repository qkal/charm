...
/** Where getSessionDBPath() actually writes: <fakeHome>/.claude/charm/sessions/ */
...
beforeAll(() => {
  ...
  sessionDBDir = join(fakeHomeDir, ".claude", "charm", "sessions");
});
...
describe("Issue #117 — Session hooks without build/session/", () => {
  test("posttooluse.mjs creates session DB via bundle (no build/ needed)", () => {
    ...
    // Bundle-first fix: DB is created in ~/.claude/charm/sessions/
