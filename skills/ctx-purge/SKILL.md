...
name: ctx-purge
description: |
  Purge the charm knowledge base. Permanently deletes all indexed content
  and resets session stats. This is destructive and cannot be undone.
  Trigger: /charm:ctx-purge
user-invocable: true
---
...
2. Call the `mcp__charm__ctx_purge` MCP tool with `confirm: true`.
...
- `/clear` and `/compact` do NOT affect any charm data.
