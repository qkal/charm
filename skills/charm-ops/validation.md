...
Could you provide:
1. Output from: `npx charm doctor` (or run `ctx-debug.sh`)
...
```
Step 1: GREP — Does it exist in charm source?
  → rg "{ENV_VAR}" src/
...
| Override | `CHARM_PLATFORM` | src/adapters/detect.ts |
...
```javascript
// WRONG — hardcoded /tmp
const tmpFile = "/tmp/charm-output.txt";

// CORRECT — uses OS temp dir
const tmpFile = path.join(os.tmpdir(), "charm-output.txt");
```
