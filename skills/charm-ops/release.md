...
3. `version-sync.mjs` syncs version to ALL 6 manifest files:
   ...
   - `.pi/extensions/charm/package.json`
...
Verify publication:
```bash
npm view charm version  # should show new version
```
...
- [ ] Verify: `npm view charm version` shows new version
...
npm unpublish charm@{BAD_VERSION}

# Or deprecate
npm deprecate charm@{BAD_VERSION} "Known issue: {description}"
