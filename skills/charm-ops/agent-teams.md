...
| Agent | Role | When |
|-------|------|------|
| **Charm Architect** | Reviews ALL changes against core architecture. Validates FTS5, MCP protocol, session continuity. Final approval gate. | Always |
...
```
You are the {Platform/Domain} {Role} for charm.
...
## Tools Available
- Use charm MCP tools (ctx_execute, ctx_batch_execute) for large output
