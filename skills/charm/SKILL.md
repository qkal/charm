...
name: charm
description: |
  Use charm tools (ctx_execute, ctx_execute_file) instead of Bash/cat when processing
...
# Charm: Default for All Large Output

## MANDATORY RULE

<charm_logic>
  <mandatory_rule>
    Default to charm for ALL commands. Only use Bash for guaranteed-small-output operations.
  </mandatory_rule>
</charm_logic>
...
**When uncertain, use charm.** Every KB of unnecessary context reduces the quality and speed of the entire session.
...
│   └── Use charm ctx_execute or ctx_execute_file
│
...
Use charm for ANY of these, without being asked:
...
4. **For files you need to EDIT**: Use the normal Read tool. charm is for analysis, not editing.
5. **For Bash whitelist commands only**: Use Bash for file mutations, git writes, navigation, process control, package install, and echo. Everything else goes through charm.
...
<sandboxed_data_workflow>
  ...
  <workflow>
    LargeDataTool(filename: "path") → mcp__charm__ctx_index(path: "path") → ctx_search()
  </workflow>
</sandboxed_data_workflow>
...
Subagents automatically receive charm tool routing via a PreToolUse hook. You do NOT need to manually add tool names to subagent prompts — the hook injects them. Just write natural task descriptions.
