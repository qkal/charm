...
if (process.platform === "win32" && process.env.npm_config_global === "true") {
  try {
    ...
    try {
      const whereOutput = execSync("where charm.cmd", {
        encoding: "utf-8",
        stdio: ["pipe", "pipe", "pipe"],
      }).trim();
      for (const line of whereOutput.split(/\r?\n/)) {
        if (line.endsWith("charm.cmd")) {
    ...
    for (const shimDir of shimDirs) {
      const expectedPkgDir = join(shimDir, "node_modules", "charm");
      ...
      if (
        ...
        if (!isSafeWindowsPath(expectedPkgDir) || !isSafeWindowsPath(actualPkgDir)) {
          console.warn(`  charm: skipping junction — path contains unsafe characters`);
        } else {
          execSync(`mklink /J "${expectedPkgDir}" "${actualPkgDir}"`, {
            shell: "cmd.exe",
            stdio: "pipe",
          });
          console.log(`\n  charm: created junction for nvm4w compatibility`);
      ...
      for (const line of whereOutput.split(/\r?\n/)) {
        if (line.endsWith("charm.cmd")) {
          ...
          if (content.includes("build\\cli.js") || content.includes("build/cli.js")) {
            ...
            console.log(`  charm: fixed stale shim at ${line}`);
          }
        }
      }
    } catch { /* best effort */ }
