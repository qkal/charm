...
# ─── 3. charm Installation ────────────────────────────────────────────

section "3. charm Installation"
...
# npm latest
NPM_LATEST="$(safe_cmd_quiet npm view charm version 2>/dev/null)"
...
# Detection logic (mirrors charm adapter selection)
if [ -n "${CHARM_PLATFORM:-}" ]; then
  DETECTED_ADAPTER="$CHARM_PLATFORM (explicit)"
...
if [ -d "$HOME_DIR/.openclaw/extensions/charm" ]; then
  printf -- '- **OpenClaw extension dir**: exists (`~/.openclaw/extensions/charm/`)\n'
else
...
FTS5_RESULT="$(safe_cmd_quiet node -e "
  try {
    ...
    db.exec(\"INSERT INTO t VALUES('charm diagnostic test')\");
...
if command -v ps &>/dev/null; then
  CTX_PROCS="$(ps aux 2>/dev/null | grep '[c]ontext-mode' | grep -v -E 'charm-dashboard|esbuild|wrangler|workerd|grep' || true)"
...
kv "Running charm processes" "$CTX_COUNT"
...
if [ "$CTX_COUNT" -gt 3 ]; then
  warn "Multiple charm processes — possible orphans"
fi
...
SESSION_DIRS=(
  "$HOME_DIR/.claude/charm/sessions"
  "$HOME_DIR/.cursor/charm/sessions"
  "$HOME_DIR/.codex/charm/sessions"
  "$HOME_DIR/.gemini/charm/sessions"
  "$HOME_DIR/.openclaw/charm/sessions"
  "$HOME_DIR/.kiro/charm/sessions"
)
TOTAL_DB_COUNT=0
...
# charm specific
kv "CHARM_SESSION_SUFFIX" "${CHARM_SESSION_SUFFIX:-unset}"
kv "CHARM_NODE" "${CHARM_NODE:-unset}"
...
SESSION_BASE="$HOME_DIR/.claude/charm/sessions"
...
NPM_TLS="$(timed 10 node -e "
  ...
  const req = https.get('https://registry.npmjs.org/charm', { timeout: 5000 }, (res) => {
