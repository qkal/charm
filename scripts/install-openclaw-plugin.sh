#!/usr/bin/env bash
# One-shot installer: register charm plugin into a local OpenClaw instance.
...
EXT_DIR="$OPENCLAW_STATE_DIR/extensions/charm"

echo "→ charm plugin installer"
...
// charm OpenClaw global extension — auto-generated by install script.
...
echo "→ clearing jiti cache for charm..."
rm -f /tmp/jiti/charm-index.*.cjs /tmp/jiti/build-openclaw-plugin.*.cjs 2>/dev/null || true
...
if [ -n "$OPENCLAW_BIN" ] && OPENCLAW_STATE_DIR="$OPENCLAW_STATE_DIR" "$OPENCLAW_BIN" plugins list 2>&1 | grep -q 'charm'; then
  echo "  ✓ charm discovered"
else
...
if (!allow.includes('charm')) allow.unshift('charm');

// Add to plugins.entries
const entries = plugins.entries ??= {};
if (!entries['charm']) entries['charm'] = { enabled: true };
...
echo ""
echo "✓ done — charm plugin installed and active"
