...
echo "=== charm OpenClaw E2E Synthetic Test ===" | tee "$LOG_FILE"
...
contextEngineId === "charm"
  ? pass("registerContextEngine('charm') called")
  : fail("registerContextEngine('charm') called", `got: ${contextEngineId}`);
...
const sessionDir = join(homedir(), ".openclaw", "charm", "sessions");
