#!/usr/bin/env node
/**
 * charm Insight — Local analytics dashboard.
  ...
  try {
    ...
    if (msg.includes("incompatible architecture") || msg.includes("dlopen")) {
      const cacheHint = process.env.INSIGHT_SESSION_DIR
        ...
        : join("~", ".claude", "charm", "insight-cache", "node_modules");
      console.error(`\n  Fix: rm -rf ${cacheHint} && charm insight`);
    } else {
...
// ── Paths ────────────────────────────────────────────────
const SESSION_DIR = process.env.INSIGHT_SESSION_DIR || join(homedir(), ".claude", "charm", "sessions");
const CONTENT_DIR = process.env.INSIGHT_CONTENT_DIR || join(homedir(), ".claude", "charm", "content");
...
function apiAnalytics() {
  ...
  // 3. Sandbox Adoption — charm MCP tool usage vs total
