/**
 * Shared session helpers for charm hooks.
...
function getWorktreeSuffix() {
  const envSuffix = process.env.CHARM_SESSION_SUFFIX;
 ...
 * Path: ~/<configDir>/charm/sessions/<SHA256(projectDir)[:16]>.db
 */
export function getSessionDBPath(opts = CLAUDE_OPTS) {
  const projectDir = getProjectDir(opts);
  const hash = createHash("sha256").update(projectDir).digest("hex").slice(0, 16);
  const dir = join(homedir(), opts.configDir, "charm", "sessions");
 ...
 * Path: ~/<configDir>/charm/sessions/<SHA256(projectDir)[:16]>-events.md
 */
 ...
 * Path: ~/<configDir>/charm/sessions/<SHA256(projectDir)[:16]>.cleanup
 */
