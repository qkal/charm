...
/**
 * Unified PreToolUse hook for charm (Claude Code)
 * Redirects data-fetching tools to charm MCP tools
 *
...
try {
  ...
  const marker = resolve(tmpdir(), `charm-healed-${myVersion}`);
  ...
  if (myVersion !== "unknown" && isInPluginCache && !existsSync(marker)) {
    ...
    try {
      ...
      // Remove any settings.json charm entries to prevent duplicate concurrent
      ...
      if (existsSync(hooksJsonPath)) {
        for (const hookType of Object.keys(allHooks)) {
          ...
          const filtered = entries.filter(
            (entry) =>
              !entry.hooks?.some(
                (h) => h.command?.includes(".mjs") && h.command?.includes("charm"),
              ),
          );
        ...
        for (const hookType of Object.keys(allHooks)) {
          ...
          for (const entry of entries) {
            ...
            // Rewrite stale charm hook paths to point to current version
            for (const h of (entry.hooks || [])) {
              if (h.command && h.command.includes(".mjs") && h.command.includes("charm") && !h.command.includes(targetDir)) {
