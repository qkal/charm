...
const _guidanceId = process.env.VITEST_WORKER_ID
  ...
  : String(process.ppid);
const _guidanceDir = resolve(tmpdir(), `charm-guidance-${_guidanceId}`);
...
export function routePreToolUse(toolName, toolInput, projectDir, platform) {
  ...
  if (canonical === "Bash") {
    ...
    // Stage 2: Charm routing (existing behavior)
  ...
  // ─── Agent: inject charm routing into subagent prompts ───
  ...
  if (
    (toolName.includes("charm") && /(?:__|\/)(ctx_)?execute$/.test(toolName)) ||
  ...
  // Cursor can also surface the tool as MCP:ctx_execute_file.
  if (
    (toolName.includes("charm") && /(?:__|\/)(ctx_)?execute_file$/.test(toolName)) ||
  ...
  if (toolName.includes("charm") && /(?:__|\/)(ctx_)?batch_execute$/.test(toolName)) {
    if (security) {
